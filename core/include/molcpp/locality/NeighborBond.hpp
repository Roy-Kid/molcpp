...
public:
    ...
    NeighborBond(unsigned int query_point_idx, unsigned int point_idx, float w, const Vec3<float>& v)
        : query_point_idx(query_point_idx), point_idx(point_idx), distance(std::sqrt(xt::sum(v * v)())), weight(w),
          vector(v)
    {}
    ...
    bool lessIdRefWeight(const NeighborBond& n) const
    {
    ...
    bool lessAsTuple(const NeighborBond& n) const
    {
        if (query_point_idx != n.query_point_idx)
        {
            return query_point_idx < n.query_point_idx;
    ...
    bool lessAsDistance(const NeighborBond& n) const
    {
        if (query_point_idx != n.query_point_idx)
        {
            return query_point_idx < n.query_point_idx;
