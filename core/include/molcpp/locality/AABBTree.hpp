...
{
    //! Default constructor
    AABBNode() : aabb(), left(INVALID_NODE), right(INVALID_NODE), parent(INVALID_NODE), skip(0), num_particles(0)
    {
        // Explicitly initialize all members
        particles.fill(0);
        particle_tags.fill(0);
    }
                                        ...
                                        unsigned int len, unsigned int parent)
{
    ...
    if (len != 2)
    {
        ...
        if (my_radius[0] > my_radius[1] && my_radius[0] > my_radius[2])
        {
            // split on x direction
            for (unsigned int i = 0; i < start_right; i++)
            {
                if (aabbs[start + i].getPosition()[0] < my_aabb.getPosition()[0])
                {
        ...
        else if (my_radius[1] > my_radius[2])
        {
            // split on y direction
            for (unsigned int i = 0; i < start_right; i++)
            {
                if (aabbs[start + i].getPosition()[1] < my_aabb.getPosition()[1])
                {
                    // if on the left side, everything is happy, just continue on
                }
                else
        ...
        else
        {
            ...
            for (unsigned int i = 0; i < start_right; i++)
            {
                if (aabbs[start + i].getPosition()[2] < my_aabb.getPosition()[2])
                {
                    // if on the left side, everything is happy, just continue on
                }
                else
    ...
    // Set the current node's basic info before recursive calls to avoid issues with reallocation
...
inline unsigned int AABBTree::allocateNode()
{
    ...
    if (m_num_nodes >= m_node_capacity)
    {
        ...
        // allocate new memory
        AABBNode* m_new_nodes = new AABBNode[m_new_node_capacity];
