...
PYBIND11_MODULE(molcpp_aabb, m) {
    ...
    py::class_<AABBNeighborQuery>(m, "AABBNeighborQuery")
        .def(py::init([](Box box, const xt::pyarray<float>& points, int leaf_size) {
                if (points.dimension() != 2 || points.shape(1) != 3) {
                    throw std::runtime_error("points must have shape (N, 3)");
                }
                // Copy into xtensor (ensures contiguous)
                xt::xtensor<float, 2> pts = points;
                return new AABBNeighborQuery(std::move(box), pts, leaf_size);
            }),
