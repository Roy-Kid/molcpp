...
#define BE_DOWNUNSPEC 16
    ...
    static int CreateAtom(Pattern *pat, AtomExpr *expr, int part, int vb)
    {
        ...
        if (pat->acount == pat->aalloc)
        {
            ...
            if (pat->atom)
            {
                ...
                std::copy(pat->atom, pat->atom + pat->aalloc - ATOMPOOL, tmp);
                delete[] pat->atom;
                pat->atom = tmp;
            }
            else
    ...
    static int CreateBond(Pattern *pat, BondExpr *expr, int src, int dst)
    {
        ...
        if (pat->bcount == pat->balloc)
        {
            ...
            if (pat->bond)
            {
                ...
                std::copy(pat->bond, pat->bond + pat->balloc - BONDPOOL, tmp);
                delete[] pat->bond;
                pat->bond = tmp;
            }
            else
    ...
    Pattern *SmartsPattern::SMARTSError(Pattern *pat)
    {
        // stringstream errorMsg;
        // errorMsg << "SMARTS Error:\n"
        //          << MainPtr << endl;
        // errorMsg << std::setw(LexPtr - MainPtr + 1) << '^' << endl;
    ...
    AtomExpr *SmartsPattern::ParseSimpleAtomPrimitive(void)
    {
        switch (*LexPtr++)
        {
        case '*':
    ...
    AtomExpr *SmartsPattern::ParseComplexAtomPrimitive(void)
    {
        Pattern *pat;
        int index;
    ...
    AtomExpr *SmartsPattern::ParseAtomExpr(int level)
    {
    ...
    BondExpr *SmartsPattern::ParseBondPrimitive(void)
    {
        char bsym = *LexPtr++;
    ...
    BondExpr *SmartsPattern::ParseBondExpr(int level)
    {
    ...
    int SmartsPattern::GetVectorBinding()
    {
        int vb = 0;
    ...
    Pattern *SmartsPattern::ParseSMARTSError(Pattern *pat, BondExpr *expr)
    {
        if (expr)
            FreeBondExpr(expr);
    ...
    static void MarkGrowBonds(Pattern *pat)
    {
        ...
        for (i = 0; i < pat->bcount; ++i)
        {
            ...
            bv.set(pat->bond[i].src, true);
            bv.set(pat->bond[i].dst, true);
        }
    }
    ...
    static int GetChiralFlag(AtomExpr *expr)
    {
        ...
        case AE_NOT:
            ...
            if (tmp1 == AL_ANTICLOCKWISE)
                return AL_CLOCKWISE;
            if (tmp1 == AL_CLOCKWISE)
                return AL_ANTICLOCKWISE;
            break;
        }
    ...
    Pattern *SmartsPattern::ParseSMARTSPart(Pattern *result, int part)
    {
        ParseState stat;
        int i, flag;
    ...
    Pattern *SmartsPattern::ParseSMARTSPattern(void)
    {
        Pattern *result;
        result = AllocPattern();
    ...
    Pattern *SmartsPattern::ParseSMARTSString(char *ptr)
    {
        Pattern *result;
    ...
    Pattern *SmartsPattern::ParseSMARTSRecord(char *ptr)
    {
        char *src;
    ...
    bool SmartsPattern::Init(const char *buffer)
    {
        delete[] _buffer;
        _buffer = new char[std::strlen(buffer) + 1];
        std::strcpy(_buffer, buffer);
    ...
    bool SmartsPattern::Init(const std::string &s)
    {
        return Init(s.c_str());
    }
