    ...
    def __call__(self):
        if self._cached_dll is None:
            ...
            if not _compatible_versions(version, __version__):
                self._cached_dll = None
                raise RuntimeError(
                    f"wrong version for libmolcpp, we want v{__version__}, "
...
def _lib_path():
    try:
        ...
        from ._external import EXTERNAL_MOLCPP_PATH

        return EXTERNAL_MOLCPP_PATH
    except ImportError:
        pass
    ...
    if sys.platform.startswith("darwin"):
        windows = False
        path = os.path.join(_HERE, "lib", "libmolcpp.dylib")
    elif sys.platform.startswith("linux"):
        windows = False
        path = os.path.join(_HERE, "lib", "libmolcpp.so")
    elif sys.platform.startswith("win"):
        windows = True
        path = os.path.join(_HERE, "bin", "molcpp.dll")
    else:
    ...
    raise ImportError("Could not find molcpp shared library at " + path)
...
def _check_dll(path):
    ...
    if python_machine == "x86":
        if machine != IMAGE_FILE_MACHINE_I386:
            raise ImportError("Python is 32-bit x86, but molcpp.dll is not")
    elif python_machine == "AMD64":
        if machine != IMAGE_FILE_MACHINE_AMD64:
            raise ImportError("Python is 64-bit x86_64, but molcpp.dll is not")
    elif python_machine == "ARM64":
        if machine != IMAGE_FILE_MACHINE_ARM64:
            raise ImportError("Python is 64-bit ARM, but molcpp.dll is not")
    else:
        raise ImportError(
