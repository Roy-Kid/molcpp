...
FAKE_INCLUDES = ROOT/"python/scripts/include"
MOLCPP_HEADER = ROOT/"molcpp/include/molcpp.h"
...
class AstVisitor(c_ast.NodeVisitor):
    ...
    def visit_Decl(self, node):
        if not node.name.startswith("molcpp_"):
            return
...
def generate_declarations():
    data = parse(MOLCPP_HEADER)

    outpath = ROOT / "python/molcpp/_c_api.py"
...
"""
        ...
        #     if name == "molcpp_create_array_callback_t":
        ...
        # file.write("\n\n")
        # callback_type = type_to_ctypes(data.types["molcpp_create_array_callback_t"])
        # file.write(f"molcpp_create_array_callback_t = {callback_type}\n")
